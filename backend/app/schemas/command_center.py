...
class DecisionRecommendationSummary(BaseModel):
    ...
    baseline_value: float = Field(..., description="Baseline target metric prediction")
    projected_value: float = Field(..., description="Projected target metric outcome")
    absolute_delta: float = Field(..., description="Absolute projected change")
    percentage_delta: float = Field(..., description="Percentage projected change")
